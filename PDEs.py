...
##PDEs for reset,remain and myopic models in 2D
...
#Myopic PDEs
def continuum_myopic_2D_params(t: float, C: np.ndarray, K: int, Nx: int, Ny: int, r: float, D: float, dx: float, dy: float):
    """
    Continuum limit PDEs for each cell stage and cell total under the remain
    model with myopic behaviour, but in 2D.
